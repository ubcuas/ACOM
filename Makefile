...
run-acom-arm: docker-arm run-dependencies
	docker run --name acom-production -it -p 0.0.0.0:5000:5000 --network=host --device=/dev/ttyACM0 --device=/dev/ttyUSB0 --add-host host.docker.internal:host-gateway --name acom-acom ubcuas/acom:arm
