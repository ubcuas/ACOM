...
class Waypoints():
    ...
    def upload_mission_wps(self, waypoints, takeoffAlt, rtl):
        ...
        self.waypoint_loader.add(
            self.generate_mission_item(0, frame, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT, 
                0, 0, 0, 0, 0
            )
        )
        seqNum += 1
        ...
        # takeoff wp
        self.waypoint_loader.add(
            self.generate_mission_item(seqNum, frame, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 
                0, 0, 0, 0, takeoffAlt
            )
        )
        seqNum += 1
        ...
        for waypoint in waypoints:   
            self.waypoint_loader.add(
                self.generate_mission_item(seqNum, frame, 
